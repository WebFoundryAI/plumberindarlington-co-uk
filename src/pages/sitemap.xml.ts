...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Darlington gets higher priority)
    const isDarlington = location.slug === 'darlington';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isDarlington ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isDarlington ? 1 : 2 + i),
    });
